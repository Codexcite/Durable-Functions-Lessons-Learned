...
{
	public class VersionedOrchestratorV2 : BaseLogged
	{
		protected readonly VersionedOptions _settings;

		// DI with the configured versions
		public VersionedOrchestratorV2(IOptions<VersionedOptions> configuredOptions) => this._settings = configuredOptions.Value;

		[FunctionName(nameof(VersionedOrchestratorV2))]
					...
					[OrchestrationTrigger] IDurableOrchestrationContext context)
		{
			...
			outputs.AddRange(await context.CallSubOrchestratorAsync<List<string>>(_settings.VersionSubOrchestrator, null));
